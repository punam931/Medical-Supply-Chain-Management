	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rsLogin12 = stRegister12
					.executeQuery("select * from user_medicine_info_order where D_Id='"
							+ mid[1] + "'");
			if (rsLogin12.next()) {
			...
			ResultSet rsLogin1 = stRegister1
					.executeQuery("select * from distribute_medicine_info where MID='"
							+ mid[0] + "'");
			if (rsLogin1.next()) {
			...
			if (oqty <= oldqty) {
				...
				st101.executeUpdate("update distribute_medicine_info set Qty='" + totalqty
						+ "' where MId='" + mid[0] + "'");
				Statement stRegister101 = con.createStatement();
				stRegister101
						.executeUpdate("update user_medicine_info_order set Status_Info='"
								+ msg
								+ "' where D_Id='"
								+ mid[1]
								+ "' and MId='" + mid[0] + "'");
